...
function normalizeErrorLog(log) {
  ...
  return {
    ...
    appSource: log.appSource || log.app_source || "backend",
    clientChannel: log.clientChannel || log.client_channel || "",
    clientPlatform: log.clientPlatform || log.client_platform || "",
    clientVersion: log.clientVersion || log.client_version || "",
...
export async function deleteProductRequest(productId) {
  ...
  return {
    productId:
      Number(response.data?.product?.id || productId) || Number(productId),
    product: normalizeProduct(response.data?.product || null)
  };
}
...
export async function deleteZoneRequest(zoneId) {
  const response = await api.delete(`/zones/${zoneId}`);
  return {
    zoneId: Number(response.data?.zone?.id || zoneId) || Number(zoneId),
    zone: normalizeZone(response.data?.zone || null)
  };
}
